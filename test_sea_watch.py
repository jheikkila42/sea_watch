...
class TestOutsideOpCoveragePriority:
    """Yö/ilta-operaation slotit priorisoidaan täytettäviksi."""

    def test_no_uncovered_slots_between_02_and_07_for_split_two_day_op(self):
        days_data = [
            {
                'arrival_hour': None,
                'arrival_minute': 0,
        ...
        for hour in range(2, 7):
            for minute in (0, 30):
                slot = time_to_index(hour, minute)
                working = sum(all_days[w][1]['work_slots'][slot] for w in daymen)
                assert working >= 1, f"Päivä 2 {hour:02d}:{minute:02d} jäi ilman daymania"
